...
class MiddlewareTest(unittest.TestCase):
    ...
    def test_download_profile(self):
        ...
        with test.override_settings(**self.override_settings):
            self.middleware.process_view(request, self.view.get, (request,), {})
            response = self.middleware.process_response(request, self.default_response)
    ...
    def test_staff_setting_no_user(self):
        self.override_settings["DJANGO_CPROFILE_MIDDLEWARE_REQUIRE_STAFF"] = True
        with test.override_settings(**self.override_settings):
            self.middleware.process_view(
                self.request, self.view.get, (self.request,), {}
            )
            self.middleware.process_response(self.request, self.default_response)
